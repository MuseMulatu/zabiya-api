...
export const requestLookIn = async (req: Request, res: Response) => {
    try {
        ...
        if (!route || !route.driver) return res.status(404).json({ error: 'Route or driver not>
        ...
        if (currentCount >= 3) {
            return res.status(429).json({ error: 'Daily look-in limit (3) reached for this rid>
        }
        ...
        const driverPushToken = "ExponentPushToken[xxxxx]"; // Fetch from route.driver.user.ex>
...
export const getHmsToken = async (req: Request, res: Response) => {
    try {
        ...
        const payload = {
            ...
            type: 'app',
            version: 2,
            jti: crypto.randomUUID(), // Replaced external package with native crypto
        };
